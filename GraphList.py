...
class GraphList:
    ...
    def load_list(self):
        ...
        valid_folders = [folder for folder in os.listdir(self.path) 
                        if os.path.isdir(os.path.join(self.path, folder)) 
                        and any(file.endswith('.json') for file in os.listdir(os.path.join(self.path, folder)))]

        for folder in reversed(valid_folders):
